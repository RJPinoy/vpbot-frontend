pipeline {
    ...
    stages {
        stage("Continuous Integration / Intégration Continue") {
            steps {
                git branch: 'main', url: 'https://github.com/RJPinoy/vpbot-frontend.git'
                sh 'npm install'
            }
        }
        stage("Continuous Delivery / Livraison Continue") {
            steps {
                sh "docker build . -t ${DOCKERHUB_USERNAME}/vpbot-frontend"
                ...
                sh "docker push ${DOCKERHUB_USERNAME}/vpbot-frontend"
            }
        }
    }
}